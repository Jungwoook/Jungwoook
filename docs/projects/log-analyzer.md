...
| 문제 상황 | 원인 | 해결 | 결과 |
| --- | --- | --- | --- |
| 여러 로그 형식 지원 시 파싱 로직이 복잡해짐 | 로그 소스별 형식과 필드 추출 규칙이 다름 | `LogParser` 인터페이스와 소스별 구현체로 분리 | 새 로그 형식 추가 시 기존 파서 변경을 최소화 |
| HTTP 계층에 분석 로직이 섞일 위험 | 컨트롤러나 서비스가 파싱과 집계를 직접 처리하면 책임이 커짐 | `app-core`, `parser`, `analysis` 모듈을 분리 | 요청 처리, 파싱, 분석 책임이 명확해짐 |
| 대용량 로그 처리 시 중간 데이터 적재 부담 | 전체 로그를 리스트로 모아 분석하면 메모리 사용량 증가 | 스트림 순회 중 `AnalysisAccumulator`로 누적 | 중간 컬렉션 없이 필요한 통계 상태만 유지 |
| 파서 판별 실패나 중복 매칭 가능성 | 내용만으로 로그 소스를 판단하기 어려운 케이스 존재 | 내용 기반 판별 후 파일명 힌트로 후보 해소 | 애매한 입력은 예외로 처리해 실패 원인을 명확화 |
